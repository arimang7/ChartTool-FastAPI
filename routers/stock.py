...
from services.stock_service import df_to_chart_json, load_data, load_news
...
async def get_stock_data(
    ...
    if df is None or df.empty:
        raise HTTPException(
            status_code=404, detail="데이터를 불러올 수 없습니다. 티커를 확인해주세요."
        )
    return df_to_chart_json(df, symbol=symbol)


@router.get("/news")
async def get_stock_news(symbol: str = Query(..., description="종목 티커")):
    """종목 관련 뉴스 조회"""
